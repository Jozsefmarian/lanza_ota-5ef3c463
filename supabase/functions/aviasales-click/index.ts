...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const userIp =
      req.headers.get("x-forwarded-for")?.split(",")[0]?.trim() || req.headers.get("x-real-ip") || "1.1.1.1";
    ...
    // Correct click endpoint — fixed base URL, NOT built from results_url
    const clicksUrl = `https://tickets-api.travelpayouts.com/search/affiliate/clicks/${encodeURIComponent(body.proposalId)}`;

    // Signature: MD5(token:marker) — same as search
    const signature = md5(`${token}:${marker}`);

    console.log("[aviasales-click] Sending click:", {
    ...
    const clickResponse = await fetch(clicksUrl, {
      ...
      body: JSON.stringify({ search_id: body.searchId }),
    });

    if (!clickResponse.ok) {
      const errorText = await clickResponse.text();
      console.error("[aviasales-click] Failed:", clickResponse.status, errorText);
      return new Response(
        JSON.stringify({
          success: false,
    ...
    console.log("[aviasales-click] Response:", JSON.stringify(clickData));
    ...
    return new Response(
      JSON.stringify({
        success: true,
        url: bookingUrl,
        rawResponse: bookingUrl ? undefined : clickData,
      }),
      {
        headers: { "Content-Type": "application/json", ...corsHeaders },
      },
    ...
    console.error("[aviasales-click] Error:", err);
    return new Response(
      JSON.stringify({
        success: false,
