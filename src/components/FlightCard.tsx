...
import { Plane, Clock, ArrowRight, Luggage, Briefcase, ChevronDown, ChevronUp, ExternalLink, Loader2 } from 'lucide-react';
...
const FlightCard: React.FC<FlightCardProps> = ({ flight, onSelect }) => {
  ...
  const handleBuy = async (e: React.MouseEvent) => {
    ...
    setBuyLoading(true);
    try {
      const result = await getFlightBookingLink({
      ...
      if (result.success && result.url) {
        ...
        // Fallback: call onSelect if no URL
        if (onSelect) onSelect(flight);
        console.warn('No booking URL returned:', result);
      }
    } catch (err) {
    ...
    <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-3 w-full">
      ...
      <div className="flex flex-col items-center flex-1 mx-2 lg:mx-6">
        ...
        <span className={`text-xs font-medium px-2 py-0.5 rounded-full mt-1 ${
          ...
          {stops === 0 ? 'Közvetlen' : `${stops} átszállás${stopAirports.length > 0 ? ` (${stopAirports.join(', ')})` : ''}`}
        </span>
      </div>
          ...
          <div className="flex items-center space-x-3 lg:min-w-[140px]">
            ...
            <div>
              ...
              <span className="text-xs text-gray-400">
                {flight.tripClass === 'Y' ? 'Economy' : flight.tripClass === 'C' ? 'Business' : flight.tripClass === 'F' ? 'First' : 'Comfort'}
              </span>
            </div>
          </div>
          ...
          <div className="flex items-center justify-between lg:flex-col lg:items-end gap-3 pt-3 lg:pt-0 border-t lg:border-t-0 lg:border-l border-gray-100 lg:pl-5 lg:min-w-[150px]">
            <div className="text-right">
              <p className="text-2xl font-bold text-gray-900">
                {Math.round(flight.price).toLocaleString('hu-HU')} <span className="text-base font-normal">{flight.currency}</span>
              </p>
              {flight.pricePerPerson !== flight.price && (
                <p className="text-xs text-gray-500">{Math.round(flight.pricePerPerson).toLocaleString('hu-HU')} {flight.currency}/fő</p>
              )}
            ...
            <div className="flex items-center gap-2 text-xs text-gray-500">
              ...
              {flight.baggageIncluded ? (
                <span className="flex items-center gap-0.5 text-green-600" title={`Poggyász${flight.baggageWeight ? ` (${flight.baggageWeight}kg)` : ''}`}>
                  ...
                  <div key={i} className="flex items-center gap-3 text-sm bg-white rounded-lg p-3">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-gray-500">
                        {formatTime(seg.departureLocal)} - {formatTime(seg.arrivalLocal)} &middot; {formatDuration(seg.durationMinutes)}
