...
export async function autocompleteSearch(query: string): Promise<AutocompleteResult[]> {
  try {
    const { data, error } = await callEdgeFunction<{ regions: AutocompleteResult[] }>(
      'ratehawk-autocomplete',
      { query }
    );
...
export async function searchFlights(params: AviasalesSearchParams): Promise<AviasalesSearchResult> {
  try {
    const { data, error } = await callEdgeFunction<AviasalesSearchResult>('aviasales-search', params);
    ...
    return { success: false, error: error.message || 'Error occurred during flight search' };
  }
}
